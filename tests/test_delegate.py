...
STD_ROSTER = [
        ("R1","老板助理","枢纽","接收任务→拆解派发→汇总回报→呈报决策",["任务派发、调度与回报校验"],("枢纽",)),
...
class TestRoleSkills(unittest.TestCase):
    ...
    def test_skills_from_shared_library_by_card_list(self):
        ...
        try:
            # 共享技能库：平铺，多角色可复用
            (agents / "skills" / "design-guide.md").write_text(
                "## 设计规范\n- 主色用品牌蓝 #2563EB\n- 间距用 8 的倍数", encoding="utf-8")
            (agents / "skills" / "copy-guide.md").write_text(
                "文案技能：标题不超过 20 字\n- 禁用感叹号", encoding="utf-8")
            ...
            self.assertIn("design-guide", p)                  # 技能名进 prompt
            self.assertIn("agents/skills/design-guide.md", p)  # 路径进 prompt
            self.assertIn("主色用品牌蓝", p)                    # 首句摘要进 prompt（供选型）
            self.assertNotIn("间距用 8 的倍数", p)              # 正文不拼进 prompt，按需读
            self.assertNotIn("copy-guide", p)                  # 卡上没登记就不装
            p2 = agent.agent_prompt("R3", "写一条推文")         # R3 装自己的
            self.assertIn("copy-guide", p2)
            self.assertIn("标题不超过 20 字", p2)
            self.assertNotIn("禁用感叹号", p2)                  # 正文按需读
            self.assertNotIn("design-guide", p2)
        finally:
