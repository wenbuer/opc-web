...
class TestSandbox(unittest.TestCase):
    ...
    def test_safe_path_rejects_escape(self):
        bad = ["../outside.txt", "项目/../../x"]
        if os.name == "nt":
            # 盘符绝对路径只有 Windows 才被当成「跑出根目录」；
            # Linux 上 "C:/Windows/system.ini" 就是个合法相对路径，不该要求抛错
            bad.append("C:/Windows/system.ini")
        for b in bad:
