...
- **自动拆解**：一句话拆成 1~3 个可独立交付的子任务，点名角色并写明期望产出；也可指定角色直派。
- **队列优先级**：待派子任务可标 高 / 普 / 低，任务挑选与执行顺序均按它排序。
- **峰谷调度**：峰时（工作日 09:00-12:00 / 14:00-18:00）单价翻倍，2 项及以上的长任务自动排到谷时开跑；「立即执行」可豁免峰时并插到队首。
- **定时任务**：每天 / 每周 / 每隔 N 分钟。

### 执行引擎与护栏

- **可替换引擎**：默认直连大模型 API，也可切换为 DSH（DeepSeek Harness）headless，主引擎失败时回退到备用引擎。
- **技能装配**：技能存放在共享技能库，角色卡登记即装配，与引擎无关。
- **过程可见**：角色卡显示当前任务；工作台四列看板（待派 / 已派 / 完成 / 阻塞）配实时事件流。
- **执行护栏**：单次运行上限为轮数 300、工具调用 320、累计输入 3000 万 token，越界即中止并按阻塞收口。
- **异常留痕**：归档、元数据与收尾链路的异常记入事件流。

### 回报、批阅与知识

- **回报落盘**：产出写 `工作区/<角色>/T-xxx-Sn-report.md` 与 `.meta.json`，完成后正文移入 `已归档/`，回报进台账。
- **批阅驱动执行**：批准即创建「执行 R0 决策」任务，驳回按批注重做；批阅台分工作内容 / 决策裁决 / 已批阅归档三段。
- **决策点分离**：每个任务都进「工作内容」，只有需要裁决的才另加一条「待决 N」。
- **知识库**：由 R1 判断是否沉淀，按主题域归档、同主题合并，每篇带 OKF front-matter。
- **每日简报**：当天任务自动汇总，写入前做结构校验。

### 成本与调度

- **三档计价**：新输入 / 缓存命中 / 输出分别计量；首页给出总额与今日。
- **运行开关**：R1 助理悬浮窗、峰时长任务排队等开关集中在「设置 → 运行开关」。

### 运行与部署

- **本地优先**：SQLite 与 Markdown，仅监听本机。
- **依赖极少**：除 `zstandard` 外均为标准库。
- **启动方式**：`run.py`；Windows 可双击 `启动控制台.bat`，macOS / Linux 用 `run.sh`。
- **主题**：深浅双主题。
