...
def agent_prompt(no: str, task_text: str, out_rel: str = "", meta_rel: str = "") -> str:
    ...
    tail = ""
    if out_rel:
        # 「可写多次」删掉了：存活心跳早就改成读引擎的会话事件（engines/dsh.py 的
        # _watch_session），文件写入不再承担心跳职责 —— 留着它只会让模型多开回合，
        # 而每一次工具调用都要把整段对话重发一遍。保留「至少落盘一次」，
        # 因为 executed 完拿不到最终文本时还要靠产出文件增长来核盘（chain._landed_evidence）。
        tail = ("\n产出文件：%s —— 至少落盘一次（把结论写进去），不必边做边追加。"
                "\n回报请在最终写入时用固定小节（便于 R0 决策）："
                "\n## 结论 —— 一两句话说清结果；"
                "\n## 依据与要点 —— 关键信息 / 依据 / 过程；"
                "\n## 需要 R0 拍板 —— 仅在确有需要 R0 拍板的事项时写这一节，且必须把每个决策点写完整：现状背景 → 可选方案 → 你的建议（不要只写一句“请 R0 拍板”，也不要写“任务含决策信号 / 请 R0 裁决 / 驳回将重新派发”这类流程空话——R0 要读到的是一句话能看懂的具体决策点）；没有需要拍板的事项就整节不写。（另：**归档口径 / 是否结案 / 状态确认 / 要不要继续**这类流程性事项不算决策点，一律不要写进这一节——那类事控制台自动处理，写了只会让 R0 白读一遍）"
    ...
    # 省用量纪律：实测失控样本的上下文从 12.9k 涨到 388k、跑了 407 轮 —— 涨的不是提问，
    # 是工具输出在对话里堆积。dsh 侧没有上下文修剪能力，唯一能压住它的就是这几句约束。
    eco = ("\n\n【省用量】每一次工具调用都要把整段对话重发一遍，少来回就是少花钱："
           "\n· 定位用 grep / 只读需要的片段，不要整篇打印；命令输出只取尾部若干行；"
           "\n· 同类操作合并成一次（一次读多个文件、一次改多处），不要一个文件一个来回；"
           "\n· 不要复述刚读到的内容 —— 读过的原文不必再写进对话或回报，只写你的结论。")
    return (head + skills + "\n\n【任务】" + task_text +
            "\n工作根目录：" + kb + "（用 / 分隔路径，知识库唯一权威根）" + eco + kb_note + tail + pj)
